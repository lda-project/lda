...
def load_reuters():
    reuters_ldac_fn = os.path.join(_data_dir, "reuters.ldac")
    return lda.utils.ldac2dtm(open(reuters_ldac_fn), offset=0)


def load_reuters_vocab():
    reuters_vocab_fn = os.path.join(_data_dir, "reuters.tokens")
...
def load_reuters_titles():
    reuters_titles_fn = os.path.join(_data_dir, "reuters.titles")
